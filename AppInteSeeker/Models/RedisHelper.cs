...
namespace RedisHelp
{
    ...
    public class RedisHelper
    {
        ...
        public RedisResult ScriptEvaluate(string script, string keyPattern)
        {
            return Do(db => db.ScriptEvaluate(LuaScript.Prepare(script), new { pattern = keyPattern }));
        }
        ...
        /// <param name="expiry">过期时间</param>
        /// <returns></returns>
        public async Task<bool> c(string key, string value, TimeSpan? expiry = default(TimeSpan?))
        {
            key = AddSysCustomKey(key);
        ...
        public void HashSet(string key, List<HashEntry> hashEntrys, CommandFlags flags = CommandFlags.None)
        {
            ...
            database.HashSet(key, hashEntrys.ToArray(), flags);
        }
        ...
        public void HashSet<T>(string key, Dictionary<string, T> dic)
        {
            ...
            {
                string vk = item.Key;
                // string json = JsonConvert.SerializeObject(item.Value, Formatting.Indented, new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
                string json = JsonConvert.SerializeObject(item.Value);
                listHashEntry.Add(new HashEntry(vk, json));
            }
            database.HashSet(key, listHashEntry.ToArray());
        }
        ...
        /// <param name="dataKey"></param>
        /// <returns></returns>
        public HashEntry[] HashKey(string key)
        {
            key = AddSysCustomKey(key);
            return Do(db =>
            {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public double? SortedSetScore(string key, RedisValue value)
        {
            key = AddSysCustomKey(key);
            return Do(redis => redis.SortedSetScore(key, value));
        }
        ...
        /// <param name="keys">rediskey</param>
        /// <returns>成功删除的个数</returns>
        public long KeyDeleteAll(string [] keys)
        {
            List<string> newKeys = keys.Select(AddSysCustomKey).ToList();
            return Do(db => db.KeyDelete(ConvertRedisKeys(newKeys)));
        }
